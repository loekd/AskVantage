...
public class ImageApiHubClient(NavigationManager navigationManager) : IImageApiHubClient, IAsyncDisposable
{
    ...
    public async Task StartAsync()
    {
        ...
        _hubConnection.On<string, QuestionGenerationResult>(nameof(IImageApiHubClient.GenerationCompleted),
            async (user, response) =>
            {
        ...
        _hubConnection.Reconnected += x => Reconnected?.Invoke(x) ?? Task.CompletedTask;
        _hubConnection.Closed += x => Closed?.Invoke(x) ?? Task.CompletedTask;
