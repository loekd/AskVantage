...
public class AzureImageOcrService(ImageAnalysisClient imageAnalysisClient, ILogger<AzureImageOcrService> logger)
    : IImageOcrService
{
    public async Task<string> GetTextAsync(byte[] image, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var result = await imageAnalysisClient.AnalyzeAsync(new BinaryData(image), VisualFeatures.Read, options,
                cancellationToken);
