...
internal class OllamaResourceLifecycleHook(ResourceNotificationService notificationService) : IDistributedApplicationLifecycleHook, IAsyncDisposable
{
    ...
    {
        foreach (OllamaResource resource in appModel.Resources.OfType<OllamaResource>())
        {
    ...
    private void DownloadModelInBackground(OllamaResource resource, CancellationToken cancellationToken)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                await notificationService.PublishUpdateAsync(resource, state => state with { State = new ResourceStateSnapshot($"Running model '{model}'", KnownResourceStateStyles.Success) });
            }
            catch (Exception ex)
            {
                await notificationService.PublishUpdateAsync(resource, state => state with { State = new ResourceStateSnapshot($"Error during download of model '{model}': {ex.Message}", KnownResourceStateStyles.Error) });
            }
    ...
    private async Task PullModel(OllamaResource resource, OllamaApiClient ollamaClient, string model, CancellationToken cancellationToken)
    {
        ...
        await notificationService.PublishUpdateAsync(resource, state => state with { State = new ResourceStateSnapshot($"Downloading model '{model}' for first use", KnownResourceStateStyles.Info) });        
    ...
    private async Task<bool> ModelExists(OllamaResource resource, OllamaApiClient ollamaClient, string model, CancellationToken cancellationToken)
    {
        IEnumerable<OllamaSharp.Models.Model> localModels = await ollamaClient.ListLocalModelsAsync(cancellationToken);
        bool exists = localModels.Any(m => m.Name.StartsWith(model));
        await notificationService.PublishUpdateAsync(resource, state => state with { State = new ResourceStateSnapshot($"Model '{model}' exists locally: {exists}", KnownResourceStateStyles.Info) });
        return exists;
    }
}
